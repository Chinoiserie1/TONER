...
export type TonerJettonMinterContent = {
    type: 0 | 1;
    uri: string;
};
export type JettonMinterConfig = { admin: Address; content: Cell; wallet_code: Cell };

export function tonerJettonMinterConfigToCell(config: JettonMinterConfig): Cell {
    return beginCell()
        .storeCoins(0)
...
export function tonerJettonContentToCell(content: TonerJettonMinterContent) {
    return beginCell()
...
export class JettonMinter implements Contract {
    ...
    static createFromConfig(config: JettonMinterConfig, code: Cell, workchain = 0) {
        const data = tonerJettonMinterConfigToCell(config);
